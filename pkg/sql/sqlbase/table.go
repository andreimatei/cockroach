...
func SplitKeysForTable(descVal *roachpb.Value, zone *zonepb.ZoneConfig) ([]roachpb.RKey, error) {
	...
	return splitKeysForTableInner(table, zone)
}

func splitKeysForTableInner(table *TableDescriptor, zone *zonepb.ZoneConfig) ([]roachpb.RKey, error) {
	...
	splits := make([]roachpb.RKey, 0, len(subzoneSplits)+1)
	splits = append(splits, tableKey)
	lastIdxID := table.NextIndexID - 1
	nextIdxPrefix := roachpb.RKey(EncodeSimpleIndexID(nil /* key */, lastIdxID+1))
	for _, s := range subzoneSplits {
		// As a special case, if the split is at the beginning of an index that's higher than any existing index, avoid
		// creating the split (since it'd be superfluous - there can be no data in between that point and the end of the
		// table). Such a split is generated by the default partition of the last index of the table.
		if s.Equal(nextIdxPrefix) {
			continue
		}

		// Prepend the table prefix to the subzone splits.
		splitKey := append(append([]byte(nil), tableKey...), s...)
		splits = append(splits, splitKey)
	}
	return splits, nil
