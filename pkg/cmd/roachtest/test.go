	...
	go func() {
		...
		for j := 0; j < count; j++ {
			for i := range tests {
				...
				r.runAsync(
					ctx, &tests[i], filterRE, runNum, func(failed bool) {
						wg.Done()
						<-sem
					})
			}
...
func (r *registry) runAsync(
	ctx context.Context, spec *testSpec, filter *regexp.Regexp, runNum int, done func(failed bool),
) {
	artifactsSuffix := ""
	if runNum != 0 {
	...
	parentDir := artifacts
	artifactsDir := filepath.Join(parentDir, teamCityNameEscape(spec.Name), artifactsSuffix)
